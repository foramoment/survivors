...
плоский** (`ENEMY_CONFIG.baseDamage = 1.5`, ×1.08 за тир, **и `DifficultyDirector`
его больше не масштабирует вообще** — `getDamageMultiplier` удалён). Поздняя
игра давит количеством и HP врагов.

```
таяние/с = Σ(враги в контакте) × armorMultiplier(armor) × contactRamp(время)
```

- **Ни капа на толпу, ни falloff.** Оба существовали, чтобы усмирить большое
  число. Геометрия ограничивает сама: вокруг игрока физически помещается 6–9
  тел, а `touching` строится по реальному пересечению.
...
| ситуация | цена |
| --- | --- |
| задеть одного врага | 3.2 HP/с — 36 секунд до смерти |
| пробежать сквозь кольцо (0.5 с) | 10 HP, 9% пула |
| кольцо на полном разгоне | 49 HP/с — 2.4 с |
...
ENEMY_CONFIG = {
    ...
    baseDamage: 1.5,       // Контактный урон В СЕКУНДУ, почти плоский (см. ContactDamage)
    damageMultiplier: 1.08, // Урон ×1.08 для следующего типа — 1.5 у первого, 3.2 у последнего
    baseXp: 1,
    xpMultiplier: 1.5,
    baseSpeed: 100,
}
