...
**Два правила зон** (`weapons/base/Zone.ts`):

1. **Расползание.** `spreadIn(seconds)` включает рампу: зона стартует с 28% от
   `fullRadius` и выезжает наружу. Включено у всего, что *льётся на пол*
   (кислота, лёд, споры, статика, огонь); выключено у телеграфленных взрывов
   (орбиталка, пси, плазма) — там ты уже посмотрел на прицел.
   Геометрия печётся против `fullRadius`, а в `draw` умножается на
   `spreadScale`, чтобы узор рос целиком, а не появлялся поверх круга.
2. **Пол на слоу — `Zone.SLOW_FLOOR = 0.35`.** Мягкий контроль может
