...
плоский** (`ENEMY_CONFIG.baseDamage = 1`, ×1.08 за тир, **и `DifficultyDirector`
...
- **Броня — кривая `K / (armor + K)`, `ARMOR_K = 25`**, а не плоское
  ...
  **перемножаются**. Плоская броня, наоборот, обнулялась к концу забега: 8
  ...
  отрицательная броня (Берсерк, −4) проходит через ту же формулу и зажата
...
| ситуация | цена |
| --- | --- |
| задеть одного врага | 2.2 HP/с — 52 секунды до смерти |
| пробежать сквозь кольцо (0.5 с) | 8 HP, 7% пула |
| простоять в кольце 3 с | 70 HP, 61% пула |
| кольцо на полном разгоне | 33 HP/с — 3.5 с |
...
| `armor`      | 0       | Кривая `K/(armor+K)`, K=25 — снижение всего входящего урона (см. ContactDamage) |
...
ENEMY_CONFIG = {
    ...
    baseDamage: 1,         // Контактный урон В СЕКУНДУ, почти плоский (см. ContactDamage)
    damageMultiplier: 1.08, // Урон ×1.08 для следующего типа — 1.0 у первого, 2.2 у последнего
    baseXp: 1,
    xpMultiplier: 1.5,
    baseSpeed: 100,
}
