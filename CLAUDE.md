...
│   ├── ContactDamage.ts  # 🩸 Укусы игроку: свой таймер у врага + бюджет укусов
...
Engine.loop
  ...
  GameManager.update(dt)
   ...
   10. resolveBites               укусы игроку (см. ContactDamage)
...
3. **Урон игроку отдельно от урона врагам (шаг 10).** `damageSystem` — только
   про врагов. Контакт идёт через `resolveBites` + `player.takeBite`, без
   глобальных i-frames.
...
Вторая редакция (непрерывная струя с `CROWD_CAP = 4`) починила броню, но
оставила две дыры: капля по 0.3 HP за кадр **не читается как урон** (нет
момента, не на что реагировать), а сто врагов стоили столько же, сколько
четыре. Реальный забег: 10 минут, 12019 килов, 5994 HP отхила, игрок стоит в
центре арены и наворачивает круги за ремкомплектами.

Как сейчас — **каждый враг кусает по своим часам**:

```typescript
// у каждого врага свой enemy.biteTimer, глобальных i-frames нет
player.takeBite(biteDamage(enemy.damage, player.stats.armor));
```

- `enemy.damage` в `ENEMY_CONFIG` — по-прежнему **урон в секунду**; укус равен
  `damage × BITE_INTERVAL × BITE_PUNCH`.
- `BITE_PUNCH = 0.7`, то есть **меньше единицы намеренно**: отдельный враг стал
  мягче, опасность идёт от их количества. Задеть одного — 4.8 HP/с (было 7),
  попасть в кольцо — 39 HP/с (было 28).
- Броня вычитается **у каждого укуса отдельно** (пол — 20%), поэтому сильна
  против кучи слабых и умеренна против одного большого.
- **Два независимых ограничителя, и нужны оба.** `biteTimer` у врага не даёт
  одному нападающему строчить очередью. Этого мало: первая версия ограничивала
  «шестью ближайшими за кадр», и 150 HP улетали за 2 секунды — куча постоянно
  расталкивается, ближайшая шестёрка **каждый кадр другая**, у каждого свежий
  таймер. Второй ограничитель — токен-бакет `biteBudget` на игроке: пополняется
  на `MAX_BITERS` за `BITE_INTERVAL`, каждый укус стоит один токен.
- Это **не** возврат i-frames. I-frames глушили *весь* входящий урон на
  фиксированное окно, поэтому одна мышь и сорок Вестников стоили одинаково.
  Бакет ограничивает только потолок: размер толпы масштабирует урон линейно
  вплоть до `MAX_BITERS` — то есть ×6 от одиночки против прежних эффективных
  ×2.5.
- `takeDamage` (с i-frames) остался для **дискретных** ударов: метеориты,
  схлопывание разлома. Не зови его для контакта — для этого `takeBite`.
- Фидбек — `GameManager.emitContactFeedback`. Укус теперь **событие**, и всё
  равно **никакой тряски камеры**: именно когда тебя обступили, надо видеть, где
  просвет, а трясущаяся арена это отнимает. Звук ограничен по частоте
  (`CONTACT_SOUND_MIN_GAP`), потому что кольцо из шести жуёт быстрее, чем ухо
  терпит «тк-тк-тк». Каждый укус отмечает красная вспышка спрайта игрока
  (`sprites.getPlayerSprite(id, frame, hurt)`), плюс ступенька на полоске HP и
  виньетка, углубляющаяся с падением здоровья.
...
1. **Урон врагам — всегда через DamageSystem** — не вызывай `enemy.takeDamage()`
   напрямую. Урон **игроку** от контакта — через `resolveBites` +
   `player.takeBite`; `player.takeDamage` только для дискретных ударов
