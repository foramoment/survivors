...
| `BurningTrailZone`     | Горящая земля (Inferno Lash, Cluster Bomb, след Void Ray) |
...
`VoidRayBeam` удалён — Void Ray теперь **ведомый** луч
(`VoidRayWeapon.SweepingLance`): захват цели, затем горящий конец луча
протаскивается к следующему врагу, разрезая всё по пути и оставляя огонь
(`BurningTrailZone`). Цель следующего рывка выбирается **не ближайшая**: из
кольца 130–430px, иначе луч дёргается между двумя врагами у тебя под ногами.
Эволюция — три рывка подряд.
...
| Void Ray | шире луч |
| Spinning Ember | +1 орб за 2 уровня |
...
Мага Бури; хочешь ведомый Void Ray — играешь Странником Пустоты. Это же даёт
...
### Kill Echo не цепляется

Убитый **эхом** помечается `Enemy.echoed` и своё эхо уже не детонирует. Без
этого одна детонация в плотной толпе на сложном стейдже (где у трупа пятизначное
HP) вычищала весь экран: каждая смерть от взрыва катала свой бросок, поколение
за поколение, без предела. Уцелевшие просто загораются — DoT не может
каскадировать внутри кадра.
